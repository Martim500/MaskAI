...
→ アプリ専用の新規IAMユーザーを作成し、このポリシーの対象外にすることで解決。
...
| リソース | 値 |
|---|---|
| Bedrock Guardrail ID | `docs/private-notes.md` 参照（環境ごとに異なる） |
| マスキング対象PII | 氏名・メールアドレス・電話番号・住所・年齢・クレジットカード番号 |
| カスタム正規表現 | 自社名・略称など（環境ごとに設定） |
| IAMユーザー | アプリ専用の最小権限ユーザー（`bedrock:ApplyGuardrail`等） |
